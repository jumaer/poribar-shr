...
class PrayerTimeService {
  ...
  Future<void> savePrayerAlarmSettings({
    ...
    try {
      ...
      // 3. Actively schedule daily alarms on the device for all 5 waqts
      await scheduleAllPrayerAlarms(settings: settings);

      // 4. Trigger feedback with high sound & strong vibration channel
      NotificationService().showPrayerAlarmNotification(
        id: 9991,
        title: 'নামাজের আযান ও অ্যালার্ম সফলভাবে চালু হয়েছে',
        body: 'প্রতিটি ওয়াক্তের নির্ধারিত সময়ে স্বয়ংক্রিয় আযান ও অ্যালার্ম বাজবে।',
  ...
  /// Actively schedules daily repeating alarms for all 5 daily prayers
  Future<void> scheduleAllPrayerAlarms({
    ...
    for (final p in prayers) {
      if (p.isAlarmEnabled) {
        final notifId = waqtIds[p.id] ?? 1000;
        await NotificationService().scheduleDailyPrayerAlarm(
          waqtNotificationId: notifId,
          waqtName: p.nameBn,
          hour: p.startTime.hour,
          minute: p.startTime.minute,
          reminderOffsetMinutes: p.reminderOffsetMinutes,
